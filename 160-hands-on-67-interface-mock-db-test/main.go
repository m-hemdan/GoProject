...
func main() {
	db := MockDatastore{ //database
		Users: make(map[int]User), // each user has values
	}

	srvc := Service{ // name of server
		ds: db,
	}

	u1 := User{ //define new user
		ID:    1,
		First: "James",
	}

	err := srvc.SaveUser(u1) //   call saveuser fun
	if err != nil {
